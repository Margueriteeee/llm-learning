...
### 3.3.2 LLaMA 
LLM 的普适架构，由Meta（前Facebook）开发的一系列大型预训练语言模型

**（1）模型架构：decoder only**
LLaMA模型的整体结构与GPT系列模型类似，只是在模型规模和预训练数据集上有所不同

LLaMA 的模型处理流程：  
- tokenizer对输入文本进行编码，转化为input_idx  
- embedding层映射到高维空间的词向量，同时通过positional embedding进行位置编码  
- 经过嵌入和位置编码得到的 hidden_states 输入到decoder层，由多个decoder block进行处理，应用masked self-attention  
- 接着进入MLP层，由两个全连接层进行进一步特征提取，先映射到中间维度并应用激活函数注入非线性，第二个全连接层把特征映射回原始维度  
- 多个block处理完后通过一个线性层，得到最终的映射，输出维度和词表维度相同，生成一个目标序列的概率分布  
- 最后通过采样或者贪婪解码等方法生成最终的输出序列  

### 3.3.3 GLM 
智谱开发的主流中文llm之一、覆盖指令理解、代码生成等多种应用场景，第一个开源的中文llm

**（1）模型架构-GPT基础上的略微修正**
GLM：清华计算机系提出，在传统CLM预训练任务（针对NLG）的基础上，加入MLM思想（针对NLU），从而构建一个在NLG和NLU上都具有良好表现的统一模型  

decoder only，相比GPT的差异：  
-  Post Norm 而非 Pre Norm。先计算残差，再计算层归一化。postnorm正则化的效果更强  
-  > 一般而言，较大体谅的llm用pre-norm的比较多，因为一部分参数直接加在后面而不需要进行正则化，正好可以防止模型的梯度爆炸或者梯度消失    
-  输出端使用单个线性层 代替 MLP。结构更简单，把更多参数放在型本身  
...
优化的自回归空白填充任务：  
- 对输入序列进行一个类似MLM的随机掩码，每次遮蔽一连串token而不是单个  
- 所以模型在预测被遮蔽的部分时即需要通过已知的上下文推理，在mask内部有需要以CLM的方式预测被连续遮蔽的token

上述方法在**预训练模型**时代具备较大的优势。  
迈入LLM时代之后，CLM在超大规模和体量的预训练上具备更大的优势